...
def _seconds_until_next_9am_central() -> float:
...
async def _daily_precompute_loop():
    """Run precompute_context on startup, then every day at 9am Central."""
    # Run once immediately on boot so the volume has fresh data
    await _run_precompute()

    while True:
        sleep_s = _seconds_until_next_9am_central()
        logger.info("[daily] next precompute in %.1f hours", sleep_s / 3600)
...
async def _run_precompute():
    try:
        logger.info("[daily] running precompute_context")
        proc = await asyncio.create_subprocess_exec(
            "python", "-m", "src.daily.precompute_context",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.STDOUT,
        )
        stdout, _ = await proc.communicate()
        logger.info("[daily] precompute done (rc=%s)", proc.returncode)
        if stdout:
            logger.info(stdout.decode()[-2000:])
    except Exception as e:
        logger.exception("[daily] precompute failed: %s", e)


@asynccontextmanager
async def lifespan(app):
    task = asyncio.create_task(_daily_precompute_loop())
    yield
    task.cancel()


# Then update your existing FastAPI line:
app = FastAPI(title="MLB Predict", version="0.2.0", lifespan=lifespan)